	...
	const bool debug)
{
  ...
  torch::Tensor out_segment = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor out_extra_features = torch::full({feature_degree, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_segment.contiguous().data<float>(),
		out_extra_features.contiguous().data<float>(),
		out_others.contiguous().data<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_segment, out_extra_features, out_others, radii, segment, geomBuffer, binningBuffer, imgBuffer);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_segment,
	const torch::Tensor& dL_dout_extra_features,
	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dsegment = torch::zeros({P,1}, means3D.options());
  torch::Tensor dL_dextra_features = torch::zeros({P, feature_degree}, means3D.options());
  ...
  {  
	  ...
	  segments.contiguous().data<float>(),
	  extra_features.contiguous().data<float>(),
	  feature_degree,
	  transMat_precomp.contiguous().data<float>(),
	  viewmatrix.contiguous().data<float>(),
	  ...
	  dL_dout_segment.contiguous().data<float>(),
	  dL_dout_extra_features.contiguous().data<float>(),
	  dL_dout_others.contiguous().data<float>(),
	  dL_dmeans2D.contiguous().data<float>(),
	  dL_dnormal.contiguous().data<float>(),  
	  dL_dopacity.contiguous().data<float>(),
	  dL_dsegment.contiguous().data<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dsegment, dL_dextra_features, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
